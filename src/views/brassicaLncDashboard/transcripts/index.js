...
const makeRows = async (getTableDataFromServerAPI) => {
    ...
    return { rows, countOfPagesInBsckend, tootalrows };
};
...
function Transcriptions() {
    ...
    const tootalrows = useSelector((state) => state.transcrsipts.table.tableData.tootalrows);
    // console.log(tootalrows, countOfPagesInBackend, pageOfPagination, numberOfRowsPerPage, searchQuery);
    ...
    const handleSubmitSearch = () => {
        ...
        const exonForSearch =
            searchQuery.exon.start && searchQuery.exon.end ? searchQuery.exon.start.concat(',', searchQuery.exon.end) : '';
        const searchItem = searchQuery.search;
        // console.log(locationForSearch, lengthForSearch, exonForSearch, searchItem, 'hhhh');
        dispatch({ type: CHANGE_TABLE_PAGINATION_PAGE, pageOfPagination: 0 });
        dispatch({
            type: TRANSCRIPT_SEARCH_SET_PATH,
            path: `/lncRNA/transcripts/?show=${numberOfRowsPerPage}&gid=${searchQuery.geneId}&tid=${searchQuery.transcriptId}&class=${searchQuery.classification}&loc=${locationForSearch}&len=${lengthForSearch}&exon=${exonForSearch}&search=${searchItem}`
        });
    };
            ...
            .then((res) => {
                ...
                dispatch({
                    ...
                    tableData: {
                        ...
                        tootalrows: res.tootalrows
                    }
                });
                // console.log(res.countOfPagesInBsckend);
            })
            ...
            {canReturnRender && (
                <>
                    ...
                    <TablePagination
                        ...
                        component="div"
                        // count={countOfPagesInBackend[0] * numberOfRowsPerPage}
                        count={tootalrows[0]}
