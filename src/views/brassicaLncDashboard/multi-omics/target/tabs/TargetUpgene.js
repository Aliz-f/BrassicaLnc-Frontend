...
function Row(props) {
    ...
    const getsubInfo = async (id) => {
        setOpen(!open);
        await axios
            .get(`${api.mainURL}/lncRNA/target/up/des/?gene_id=${id}`)
            .then((res) => {
                // console.log(res);
...
function TargetUpgene() {
    ...
    useEffect(async () => {
        makeRows(
            `${api.mainURL}/lncRNA/target/up/?per_page=${rowsPerPage}&page=${page + 1}&query=${queryId}&len_query=${
